...
// This is the HTTP/2 plumbing howsmyssl uses on Go toolchains older than
// 1.27, where net/http could only dispatch ALPN through TLSNextProto, whose
// signature names the concrete *crypto/tls.Conn type our forked tls1266
// stack can't produce. We route ALPN ourselves and hand "h2" connections to
// golang.org/x/net/http2, which will serve any net.Conn.
//
// Go 1.27 removed that restriction; see server_stdh2.go, which is the
// implementation that actually builds today. Keep the two in sync if you
// change Serve/Shutdown/Close semantics.
...
func (s *Server) serveH2(c *Conn) {
	...
	go func() {
		...
		// BaseConfig hands x/net's HTTP/2 server h1's timeouts, MaxHeaderBytes,
		// ConnState, and ErrorLog. The conn is registered on s.h2's own
		// connection set, which ConfigureServer wired into h1's shutdown, so
		// h1.Shutdown still reaches it with a GOAWAY.
